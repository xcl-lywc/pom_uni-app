		...
		<view class="uni-steps-items" :class="'uni-steps-' + direction">
			<view class="uni-steps-item" v-for="(item,index) in steps" :key="index" :class="[item.status ? 'uni-steps-' + item.status : '']">
				<view class="uni-steps-item-title-container" :style="{color:item.status === 'process' ? activeColor : ''}">
					<view v-if="item.status === 'process'" style="display: flex;;">
						<view style="flex: 8;">
							<view class="uni-steps-item-title marr30" style="font-size: 32upx;">{{ item.title }}</view>
							<view class="uni-steps-item-desc marr30" style="font-size: 32upx;" v-if="item.desc">{{ item.desc}}</view>
							<view class="uni-steps-item-title" style="font-size: 25upx;">{{ item.time }}</view>
						</view>
						<!-- <view class="todo" @click="todo()">{{todoString}}</view> -->
					</view>
					<view v-else>
						<view class="uni-steps-item-title marr30" style="font-size: 32upx;">{{ item.title }}</view>
						<view class="uni-steps-item-desc marr30" style="font-size: 32upx;" v-if="item.desc">{{ item.desc}}</view>
