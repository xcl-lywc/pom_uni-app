<template>
	<view class="uni-card" :class="isFull === true || isFull === 'true' ? 'uni-card--full' : ''" @click="onClick">
		...
		<view :class="{'uni-card__footer':true,'uni-card-pad30':isFull === true}" v-if="note">{{note}}
			...
			<view v-if="rightNoteMsgNum" class="fr uni-color-666 overflow-hidden">
				<view class="fl" style="margin-top: 2upx;"><fa-icon type="comments" size="18" color="#666" class="uni-marr-10"></fa-icon>{{rightNoteMsgNum}}</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		props: {
			...
			rightNoteMsgNum: String, //rightTips
