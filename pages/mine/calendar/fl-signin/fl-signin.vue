	...
	<view id="fl_signin"> 
		<view class="header">
			...
			<template v-if="choose_date">
				<view class="left" @click="date_change('last')">
					<text>上月</text>
				</view>
				<view class="right false" v-if="is_today_date">
					<text>下月</text>
				</view>
				<view class="right" @click="date_change('next')" v-else>
					<text>下月</text>
				</view>
			</template>
		...
		<view class="section">
			...
			<view class="days">
				<block v-for="(item,index) in days" :key="index">
					...
					<!-- 如果当前日期是今天的日期 -->
					<template v-else-if="is_today_date">
						<template v-if="index<day_index">
							...
							<template v-else-if="repair">
								<view class="day" @click="repair_fun(item,index)">
									<text>{{item.day}}</text> 
								</view>
							</template>
							<template v-else>
					...
					<template v-else>
						<template v-if="index>=first_index&&index<last_index">
							<template v-if="item.click">
								<view class="day true">
									<text>{{item.day}}</text>
									<text>已签</text>
					...
					<!-- 下月日期 -->
					<template v-if="index>=last_index">
	...
	export default {
		...
		methods: {
			...
			date_change(str) {
				...
				this.$emit('date_change', year + "-" + month + "-01");
			},
			// 准备
			ready() {
				...
				if (current_date) {
					...
					// 判断当前日期是否就是今天的日期
					if (arr[0] == year && arr[1] - 1 == month) {
						is_today_date = true;
					}
					current_date = [arr[0], arr[1] - 1, day];
				} else {
			...
			start() {
				...
				let days = [],
					// 当前日期的时间对象
					new_date = new Date(current_date[0], current_date[1], current_date[2]),
					year = new_date.getFullYear(),
					month = new_date.getMonth(),
					day = new_date.getDate();
				// 记录当前日期的年月日
				...
				// 记录今天的下标
				this.day_index = this.first_index + day - 1;
