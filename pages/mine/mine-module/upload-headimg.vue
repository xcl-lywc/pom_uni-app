...
<script>
import weCropper from '../../../static/js/weCropper.js';
...
// 		},
		...
		getCropperImage() {
			let _this = this;
			let pathurl = this.$mainUtils.returnServer() + 'sys/ueditor/exec?action=uploadimage';//上传服务器地址
						...
						success: res => {
							...
							if(res){
								if(res.statusCode === 200){
									imgPath = JSON.parse(res.data).path
								}
							}
							imgPath.replace(/\?/g, '--why--').replace(/\&/g, '--and--').replace(/=/g, '--equal--');
							uni.redirectTo({
							  url: "/pages/mine/mine-module/base-info-edit?title=picture&isPic=2&data="+imgPath
							})
							uni.showToast({
								title: '上传成功',
								icon: 'success',
