<template>
	<view>
		<scroll-view v-show="!isShowTree" @scrolltolower="scorllTolower" style="height: 100vh;" scroll-y="true">
		...
		<my-tree v-show="isShowTree" class="marb20" :paramsData="treeParamsData" @treeValData="treeValDataFun"></my-tree>
	</view>
</template>
...
<script>
	...
	import myTree from "./task-component/tree.vue"
	export default {
		components: {
			uniSegmentedControl,
			uniCard,
		...
		onLoad(option) {
			...
			// --------- 不同类型，变为不同的标题 -----------
			let pageType = uni.getStorageSync('taskTypePage'); 
			let title = uni.getStorageSync('taskTypePageName'); 
		...
		computed: {
			ctime(){
				return function(date) {
					if (date)
						return this.$mainUtils._convertDate(date, 'yyyy-MM-dd hh:mm:ss')
					else return "未知";
				}
			},
			cnote(){
		...
		methods: {
			...
			getTaskList(init) {
				...
				this.$mainUtils.request(requestUrl, this.searhForm, "GET", res => {
					if (res) {
						...
						// console.info(JSON.stringify(this.listData))
			 ...
			 */
			saveTree() {
				this.$mainUtils.request("t_tasks/turn/" + this.treeParamsData.processId + "?userId=" + this.treeParamsData.valueId,
					'', 'POST', (res) => {
