    ...
    <scroll-view @scrolltolower="scrollToLower" scroll-y="true">
			<view v-for="(item, index) in cardData.rows" :key="index" class='uni-mart-20'>
				<uni-card 
				  ...
				  <view><text class="uni-font-32">会议名称：{{ item.applicationName }}</text></view>
					<view><text class="uni-font-28">请假理由：{{ item.reason }}</text></view>
				</uni-card>
			</view>
  ...
  export default {
    ...
    methods: {
      ...
      getList(init) {
        ...
        this.$mainUtils.request("m_info/leave", params, 'GET', res => {
          if (res) {
       ...
       */
      cardClicked(item) {
        uni.navigateTo({url: "/pages/mine/org-life/leave/my-leave-process?taskId="+item.taskId});
      },
      /**
       * 滑动到页面底部 分页增加 并查询
       */
